...
class Kizano_Auth_Adapter_Login implements Zend_Auth_Adapter_Interface
{
    ...
    public function authenticate()
    {
        ...
        if ($hash === $user['password']) {
            if ($user['active']) {
                ...
                $this->_login['type']       = $this->getUserType($user['user_id']);
                // For the purposes of this login and the ACL setup, we only need to assign the first
                // character as that represents the role the user is playing.
                $this->_login['type']       = strToUpper($this->_login['type']{0});
                $this->_login['email']      = $user['email'];
                $this->_login['active']     = (bool)$user['active'];
                $this->_login['username']   = $user['username'];
                $this->_login['name']       = array('first' => $user['fname'], 'last' => $user['lname']);
                $result = new Zend_Auth_Result(
    ...
    {
        return Model_DbTable_UserGroups::getInstance()
            ->getGroupNameByUserId($user_id);
    }
