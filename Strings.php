...
class Kizano_Strings
{
    ...
    public static function sluggify($name)
    {
        ...
        return preg_replace('#[^\s\w\d\./]+#i', '-', preg_replace('/(\s+)/', '_', strToLower(trim($name))));
    }
    ...
    public static function hashPass($plaintext, $salt)
    {
        return strToUpper(hash('sha256',
            $salt . self::SALT . hash('haval256,5',
                $salt . $plaintext . self::SALT . hash('tiger160,4',
                    $salt . $plaintext . self::SALT
                ) . $salt
            ) . $salt
        ));
    }
    ...
    public static function getFlash()
    {
        ...
        $result = isset($session->flash)? $session->flash: array();
        unset($session->flash);
        return $result;
    }
     ...
     * @return         void
     */
    public static function flash($title, $message, $level)
    {
        $session = Zend_Registry::get('session');
        $session->flash[] = array(
            'title'     => $title,
