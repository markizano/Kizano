...
class Kizano_Array
{
    ...
    /**
     *  Casts ID's as their integer data type.
     *  
     *  @param $toCast  Array   The array to cast.
     *  
     *  @return Array
     */
    public static function castID(array $toCast)
    {
        if (!is_array($toCast)) {
            return $toCast;
        }
        foreach ($toCast as $key => $value) {
            if (is_array($value)) {
                $toCast[$key] = self::castId($value);
            } else {
                if (strIpos($key, '_id') !== false || $key == 'created_by' || $key == 'updated_by') {
                    $toCast[$key] = (int)$value;
                }
            }
        }
        return $toCast;
    ...
    /**
     *  Extracts the values of a particular key from an array.
     *  
     *  @param $array   Array   The array to extract
     *  @param $id      Mixed   The named key to match.
     *  
     *  @return Array
     */
    public static function extractID(array $array, $id)
    {
        $result = array();
        foreach ($array as $a) {
            if (isset($a[$id])) {
     ...
     *  @return Array
     */
    public static function strip_null($array)
    {
        return array_filter($array, array(self, '_not_null'));
    }

    /**
     *  Internal function to reverse the value of is_null
     *  
     *  @param $value   Mixed   The value to test.
     *  
     *  @return boolean
     */
    protected static function _not_null($value)
    {
